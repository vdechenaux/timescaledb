						  ...
						  MemoryContext per_tuple_mcxt)
{
	...
	// Chunk *osm_chunk = chunk_get_osm_chunk( CurrentMemoryContext, hs->hypertable_id);
	// int osm_chunk_id = osm_chunk ? osm_chunk->fd.id : 0;
	/*
							 ...
							 LOCKMODE chunk_lockmode, unsigned int *numchunks)
{
	...
	for (i = 0; i < unlocked_chunk_count; i++)
	{
		...
		if (ts_chunk_lock_if_exists(chunk->table_id, chunk_lockmode))
		{
			...
			if (chunk->relkind == RELKIND_FOREIGN_TABLE && !chunk->fd.osm_chunk)
				remote_chunk_count++;
