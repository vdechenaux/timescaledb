...
static void set_toast_tuple_target_on_compressed(Oid compressed_table_id);
static void set_statistics_on_compressed_table(Oid compressed_table_id);
static void create_compressed_table_indexes(Chunk *chunk, CompressionSettings *settings);
...
compression_chunk_create(Chunk *src_chunk, Chunk *chunk, List *column_defs, Oid tablespace_oid)
{
	...
	set_statistics_on_compressed_table(chunk->table_id);
	set_toast_tuple_target_on_compressed(chunk->table_id);

	create_compressed_table_indexes(chunk, settings);
...
static void
create_compressed_table_indexes(Chunk *chunk, CompressionSettings *settings)
{
	IndexStmt stmt = {
		.type = T_IndexStmt,
